jQuery( document ).ready( function( $ ) {
	...
	if ( $( '#old-post-notice-old-posts-table' ).length > 0 ) {
		...
		oldPosts.done( function( response ) {
			...
			var oldPosts = JSON.parse( response );
			...
			for ( var i = 0, len = oldPosts.length; i < len; ++i ) {
				...
				var oldPostsRows = oldPostsRows + '<tr><td><a href="' + oldPost.url + '" target="_blank">' + oldPost.title + '</a></td><td>' + oldPost.published + '</td><td>' + oldPost.modified + '</td>';
			...
			if ( oldPostsExist == true ) {
				...
				$( '#old-post-notice-old-posts-table tbody' ).html( oldPostsRows );

			} else {

				$( '#old-post-notice-old-posts-table tbody' ).html( '<tr><td colspan="100%">' + __( 'No old posts.', 'old-post-notice' ) + '</td></tr>' );
